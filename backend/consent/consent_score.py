...
def build_gdpr_assessment(
    *,
    ...
    `cookie_summary` is optional: pass it when Set-Cookie headers were
    captured for this page (see consent.cookies.analyze_cookies) to get
    a real cookies_blocked_pre_consent verdict; omit it and that check
    reports None ("not evaluated") rather than a guessed pass.
    ...
    checks.append(GdprCheck(
        "consent_banner", GDPR_CHECK_LABELS["consent_banner"], banner_detected,
        "A consent banner/CMP was found on the page." if banner_detected
    ...
    if cookie_summary is None:
        cookies_check = GdprCheck(
            "cookies_blocked_pre_consent", GDPR_CHECK_LABELS["cookies_blocked_pre_consent"], None,
            "Not evaluated — no cookie data (Set-Cookie headers) was supplied for this page.",
        )
    else:
        non_essential = [c for c in cookie_summary.cookies_detected if c.get("category") != "essential"]
        cookies_ok = len(non_essential) == 0
        cookies_check = GdprCheck(
            "cookies_blocked_pre_consent", GDPR_CHECK_LABELS["cookies_blocked_pre_consent"], cookies_ok,
            "No non-essential cookies were observed on page load." if cookies_ok
            else f"{len(non_essential)} non-essential cookie(s) observed on page load "
                 f"({', '.join(sorted({c.get('name', '') for c in non_essential})[:5])}).",
        )
