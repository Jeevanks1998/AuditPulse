...
async def start_audit(
    ...
    # Phase 4: the only "audits" module action a role can be blocked from
    # is kicking off a new one — Viewer is the one role without it (see
    # config/permissions.py). Reading existing audits (stats/recent/by id,
    # below) stays open to every authenticated user, Viewer included.
    current_user: User = Depends(require_module("audits")),
):
...
async def audit_region_summary(
    db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """GDPR / CCPA / Not Assessed counts across the user's completed audits — see
    services.audit_service.compute_region_summary. Kept under /audits (alongside
    /stats and /recent, which dashboard.js already calls) rather than only under
    /dashboard, so the dashboard page's existing Api.audits.* pattern covers it too."""
    return await audit_service.compute_region_summary(db, current_user)
